...
Iterator *CLI::blocknestedloopjoin(Iterator *input)
{
    ...
    // Create Join
    BNLJoin *join = new BNLJoin(input, right, cond, (unsigned)atoi(std::string(token).c_str()));
...
Iterator *CLI::gracehashjoin(Iterator *input)
{
    ...
    // Create Join
    GHJoin *join = new GHJoin(input, right, cond, (unsigned)atoi(std::string(token).c_str()));
...
RC CLI::createCondition(const std::string tableName, Condition &condition, const bool join, const std::string joinTable)
{
    ...
    case TypeVarChar:
        ...
        break;
    case TypeInt:
        num = atoi(std::string(token).c_str());
        memcpy((char *)value.data, &num, sizeof(int));
        break;
    case TypeReal:
        floatNum = atof(std::string(token).c_str());
...
RC CLI::createTable()
{
    ...
    // std::cout << "create table for <" << name << "> and attributes:" << std::endl;
    // for (std::std::vector<Attribute>::iterator it = table_attrs.begin() ; it != table_attrs.end(); ++it)
    //    std::std::cout << ' ' << it->length;
    //  std::cout << std::endl;
...
RC CLI::addAttribute()
{
    ...
    // convert attributes to std::vector<std::string>
...
RC CLI::dropTable()
{
    ...
    // convert attributes to std::vecto<string>
    std::vector<std::string> stringAttributes;
...
RC CLI::load()
{
    ...
    // read file
    ifstream ifs;
    std::string file_url = DATABASE_FOLDER "../data/" + fileName;
    ...
    while (ifs.good())
    {
        ...
        // for (std::std::vecto<Attribute>::iterator it = attrs.begin() ; it != attrs.end(); ++it)
...
RC CLI::insertTuple()
{
    ...
    while (token != NULL)
    {
        ...
        if (attributes.at(index).name.compare(std::string(token)) != 0)
...
RC CLI::printTable(const std::string tableName)
{
    ...
    // convert attributes to std::vector<string>
    std::vector<std::string> stringAttributes;
    for (std::std::vector<Attribute>::iterator it = attributes.begin(); it != attributes.end(); ++it)
...
RC CLI::printOutputBuffer(std::vector<std::string> &buffer, uint mod)
{
    ...
    for (uint i = 0; i < mod; i++)
    {
        std::cout << setw(maxLengths[i]) << left << buffer[i] << DIVISOR;
    ...
    for (uint i = startIndex; i < buffer.size(); i++)
    {
        if (i % mod == 0)
            std::cout << std::endl;
        std::cout << setw(maxLengths[i % mod]) << left << buffer[i] << DIVISOR;
    }
    std::cout << std::endl;
    delete[] maxLengths;
    return 0;
